...
Название переменных изменять нельзя, а вот их значение можно как вздумается (изменять можно только то, что в двойных кавычках помле двоеточия). Когда я делал перевод на русский язык, я сделал его так:
...
Его мы не трогаеи, а берём все значения переменных, что ниже. Название переменных изменять нельзя, а вот их значение можно как вздумается (изменять можно только то, что в двойных кавычках помле двоеточия). Там могут встречаться такие наборы символов, как '\n', '\n\n' и '%AMOUNT%'. \n - это переход на следующую строку, \n\n, как вы поняли на две строки, а %AMOUNT% переводить нельзя, это тоже переменная. Вот пример:
